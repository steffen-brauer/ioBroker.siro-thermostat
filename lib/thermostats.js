...
class Device{
    constructor(adapter, params){
        ...
        const { id, key, version, name, timeout, ip } = params;
        this.id = id;
        this.ip = ip;
        this.key = key;
    ...
    async initStates(){
        ...
        const states = ['connected', 'info', 'ip'].concat(Object.values(DPSMapping));
    ...
    handleConnected(){
        ...
        this.adapter.setStateAsync(this.stateName('connected'), {val: true, ack: true});
        this.adapter.setStateAsync(this.stateName('ip'), {val: this.device.device.ip, ack: true});
        this.adapter.connectionStatus(true);
    }

    handleConnect(){
        this.device.connect();
        this.adapter.setStateAsync([this.id, 'info'].join('.'), { val: '', ack: true});
    }
